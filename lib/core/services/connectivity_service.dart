    ...
    try {
      ...
      for (int attempt = 0; attempt < 2; attempt++) {
          ...
          try {
            ...
            final result = await InternetAddress.lookup(host)
                .timeout(
                  ...
                  onTimeout: () {
                    _logger.debug('ConnectivityService: DNS lookup for $host timed out after 10 seconds (attempt ${attempt + 1})');
            ...
            _logger.debug('ConnectivityService: $host lookup timed out (attempt ${attempt + 1})');
            failedHosts++;
          } catch (e) {
            _logger.debug('ConnectivityService: Error checking $host (attempt ${attempt + 1})', e);
            failedHosts++;
          }
        }
        ...
        if (failedHosts == hosts.length) {
          ...
          _logger.debug('ConnectivityService: No internet connection - all ${hosts.length} hosts failed after retries');
          return false;
        }
      }
      
      _logger.debug('ConnectivityService: No internet connection after retries');
      return false;
    } catch (e) {
      // On any outer error, assume no connection (be strict)
      _logger.debug('ConnectivityService: Error checking connectivity', e);
      return false;
    }
  }
}
